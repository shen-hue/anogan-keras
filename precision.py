...
qurey= np.load('result_artificial/X_test.npy')
pred = np.load('result_artificial/test_pred.npy')
diff = np.load('result_artificial/test_diff.npy')

shap_value = np.load('result_artificial_classification/shapvalue.npy')
mean_shap_normal = np.mean(np.abs(shap_value[:255,:]),axis=0)
mean_shap_anomaly = np.mean(np.abs(shap_value[255:,:]),axis=0)
...
# false positive,true positive index
