...
result = np.load('result_artificial/limevalue_based_all.npy')
...
for j in range(len(X_test_standard)):
    ...
    idx = j
    explainer = lime.lime_tabular.LimeTabularExplainer(X_test_standard,feature_names=range(6),
    ...
    for i in range(6):
        ...
        result[j][3][number] = lime_value.as_list()[i][1]     # need to modified code in lime library(see in notion 23th)

np.save('result_artificial/limevalue_based_all', result)
