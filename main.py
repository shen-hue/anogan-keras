from __future__ import print_function

import matplotlib

matplotlib.use('PDF')

import os
import cv2
import numpy as np
import matplotlib.pyplot as plt
from keras.datasets import mnist
import argparse
import anogan
import keras
import pandas as pd
from load_data import sine_data_generation, anomaly_sine_data_generation
from sklearn import preprocessing
from sliding_window import unroll, assign_ano

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

parser = argparse.ArgumentParser()
parser.add_argument('--img_idx', type=int, default=14)
parser.add_argument('--label_idx', type=str, default=7)
parser.add_argument('--mode', type=str, default='test', help='train, test')
args = parser.parse_args()

### 0. prepare data

### 0.2 load credit fraud data
# tmp = pd.read_csv("creditcard.csv", encoding='gbk', header=None)
# X_train = tmp.iloc[1:10000, 1:-2].values.astype(np.float)
# y_train = tmp.iloc[1:10000, -1].values.astype(np.float)
# X_test = tmp.iloc[10000:, 1:-2].values.astype(np.float)
# y_test = tmp.iloc[10000:, -1].values.astype(np.float)

### 0.2 load yahoo data
tmp_train = pd.read_csv("yahooA3_without_anomalies.csv")
train = np.asarray(tmp_train.iloc[:,1])
tmp_test = pd.read_csv("yahooA3.csv")
X_test = np.asarray(tmp_test.iloc[:,1])
y_test = np.asarray(tmp_test.iloc[:,2])
X_train_dataset = unroll(train)
X_test_dataset, y_test_dataset = unroll(X_test,labels=y_test)
y_test_dataset = assign_ano(y_test_dataset)
#### reshape
X_train_dataset = X_train_dataset[:,:,None]
X_test_dataset = X_test_dataset[:,:,None]


X_train_dataset = (X_train_dataset-np.min(X_train_dataset))/(np.max(X_train_dataset)-np.min(X_train_dataset))
X_test_dataset = (X_test_dataset-np.min(X_test_dataset))/(np.max(X_test_dataset)-np.min(X_test_dataset))


# ## 0.3.1 show the training data and test data
#
# [plt.plot(np.arange(sequence), X_train[1][:,s]) for s in range(sequence)]
# plt.title('training data')
# plt.savefig('result_sin/train_data')
# [plt.plot(np.arange(sequence), X_test[801][:,s]) for s in range(sequence)]
# plt.title('test data')
# plt.savefig('result_sin/test_data')

# X_test_original = X_test.copy()

# X_train = X_train[y_train == 1]       # in mnist data number"1" as normal data
# X_train = X_train[y_train == 0]         # in credit fraud data 0 as normal data





print('train shape:', X_train_dataset.shape)

### 1. train generator & discriminator
if args.mode == 'train':
    Model_d, Model_g = anogan.train(64, X_train_dataset)

### 2. test generator
generated_img = anogan.generate(25)




### 3. other class anomaly detection

def anomaly_detection(test_img, g=None, d=None):
    model = anogan.anomaly_detector(g=g, d=d)
    ### only for simple model credit fraud
    ano_score, similar_img = anogan.compute_anomaly_score(model, test_img.reshape(1, 100), iterations=500, d=d)

    ### only for simple model credit fraud
    np_residual = test_img.reshape(100,1) - similar_img.reshape(100,1)

    # np_residual = (np_residual + 2)/4


    return ano_score, test_img.reshape(100,1), similar_img.reshape(100,1), np_residual


### compute anomaly score - sample from strange image

n_test = X_test_dataset.shape[0]
X_test_l = X_test_dataset.shape[1]
m = range(n_test)  # X_test.shape[0]
score = np.zeros((n_test, 1))
# qurey = np.zeros((n_test, X_test_l, X_test_w, 1))
qurey = np.zeros((n_test, X_test_l, 1))
pred = np.zeros((n_test, X_test_l, 1))
diff = np.zeros((n_test, X_test_l, 1))
for i in m:
    # img_idx = args.img_idx
    # label_idx = args.label_idx
    test_img = X_test_dataset[i]
    # test_img = np.random.uniform(-1,1, (28,28,1))

    start = cv2.getTickCount()
    score[i], qurey[i], pred[i], diff[i] = anomaly_detection(test_img)
    time = (cv2.getTickCount() - start) / cv2.getTickFrequency() * 1000
    # print ('%d label, %d : done'%(label_idx, img_idx), '%.2f'%score, '%.2fms'%time)
    print("number: ", i, "score:", score[i])

np.save('result_yahoo_LSTM/yahoo_test_score', score)
np.save('result_yahoo_LSTM/yahoo_test_qurey', qurey)
np.save('result_yahoo_LSTM/yahoo_test_pred', pred)
np.save('result_yahoo_LSTM/yahoo_test_diff', diff)
np.save('result_yahoo_LSTM/X_test', X_test_dataset)
np.save('result_yahoo_LSTM/y_test', y_test_dataset)
np.save('result_yahoo_LSTM/X_train', X_train_dataset)
# np.save('result_credit_NN/y_train', y_train)

# cv2.imwrite('./qurey.png', qurey)
# cv2.imwrite('./pred.png', pred)
# cv2.imwrite('./diff.png', diff)

###### histogram

plt.hist(score, 40)
plt.savefig('result_yahoo_LSTM/histogram')
#