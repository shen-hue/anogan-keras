...
idx = 0
df = pd.DataFrame(data=test_pred[idx], index= range(13),columns=['reconstruction_loss'])
...
for i in top_5_features.index:
    ...
    shaptop5features[str(i)] = pd.DataFrame(shap_values[feature_index])
